...
export default function AudioControlBar({ 
  ...
  return (
    <footer className="fixed bottom-0 left-0 right-0 z-50 h-[80px] bg-[#050508] border-t border-white/5 flex items-center justify-between px-6 pointer-events-auto" role="contentinfo">
      ...
      <div className="flex items-center gap-4 min-w-[200px]">
        <div className="flex flex-col">
          ...
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-slate-500 font-mono tracking-tighter uppercase whitespace-nowrap">Node_{Math.random().toString(36).substring(7).toUpperCase()}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-2">
          <button 
            ...
            className={cn(
              ...
              isMicrophoneEnabled ? "bg-[#1E1E1E] text-white hover:bg-white/10" : "bg-red-500 text-white"
            )}
          >
          ...
          <button 
            ...
            className={cn(
              "w-10 h-10 rounded-full flex items-center justify-center transition-all",
              isCameraEnabled ? "bg-[#1E1E1E] text-white hover:bg-white/10" : "bg-red-500 text-white"
            )}
          >
        ...
        <div className="flex items-center gap-2">
           ...
           <div className="relative">
             <button 
               ...
               className={cn("w-10 h-10 rounded-full bg-[#1E1E1E] text-white hover:bg-white/10 flex items-center justify-center transition-all", showEmojiMenu && "bg-white/10")}
             >
           ...
           <button 
             ...
             className={cn("w-10 h-10 rounded-full bg-[#1E1E1E] text-white hover:bg-white/10 flex items-center justify-center transition-all", isScreenSharing && "text-blue-400")}
           >
           ...
           <button 
             onClick={toggleHand}
             className={cn("w-10 h-10 rounded-full bg-[#1E1E1E] text-white hover:bg-white/10 flex items-center justify-center transition-all", isHandRaised && "text-amber-400")}
           >
             ...
             <Drawer.Portal>
               ...
               <Drawer.Content className="fixed bottom-0 left-0 right-0 max-h-[85vh] bg-[#0A0A0F] border-t border-white/10 rounded-t-[2.5rem] p-8 flex flex-col gap-6 z-[100] outline-none shadow-2xl">
                 ...
                 <div className="grid grid-cols-2 md:grid-cols-3 gap-4 max-w-4xl mx-auto w-full">
                    ...
                    <button className="p-6 rounded-3xl bg-white/5 border border-white/5 hover:bg-white/10 transition-all flex flex-col items-center gap-3">
        ...
        <button 
          onClick={onLeave}
          className="ml-4 h-11 px-6 rounded-full bg-red-500 hover:bg-red-600 text-white flex items-center justify-center transition-all shadow-lg active:scale-95"
        >
      ...
      <div className="flex items-center gap-2 min-w-[200px] justify-end">
        <button 
           ...
           className={cn("w-10 h-10 rounded-full flex items-center justify-center transition-all", activeTab === 'oracle' ? "bg-white/10 text-white" : "text-slate-400 hover:bg-white/5")}
        >
          <Target className="w-5 h-5" />
        </button>
        <button 
           onClick={() => onToggleTab?.('participants')}
           className={cn("w-10 h-10 rounded-full flex items-center justify-center transition-all relative", activeTab === 'participants' ? "bg-white/10 text-white" : "text-slate-400 hover:bg-white/5")}
        >
          <Users className="w-5 h-5" />
          <span className="absolute top-1 right-1 w-4 h-4 bg-[var(--accent-primary)] rounded-full text-[9px] font-black flex items-center justify-center text-white">{participants.length}</span>
        </button>
        <button 
           onClick={() => onToggleTab?.('chat')}
           className={cn("w-10 h-10 rounded-full flex items-center justify-center transition-all", activeTab === 'chat' ? "bg-white/10 text-white" : "text-slate-400 hover:bg-white/5")}
        >
          <MessageSquare className="w-5 h-5" />
        </button>
        <button className="w-10 h-10 rounded-full text-slate-400 hover:bg-white/5 flex items-center justify-center transition-all">
